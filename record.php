...
<body>
    ...
    if (isset($_POST['submit'])) {
            ...
            if($category_id == "NULL"){
                ...
                $sql = "INSERT INTO `record` (`user_id`, `pertemuan`, `link`) VALUES ('$user_id', '$pertemuan', '$link')";
            }else{
                /* insert to ini bisa diambil dari phpmyadmin, cek aja */
                $sql = "INSERT INTO `record` (`user_id`, `category_id`, `pertemuan`, `link`) VALUES ('$user_id', '$category_id', '$pertemuan', '$link')";
            }
            ...
            <div class="col-lg-10">
                ...
                <form class="form" method="POST">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="pertemuan" placeholder="Pertemuan keberapa nih..." name="pertemuan">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="link" rows="3" placeholder="Masukin Link nya disini..." name="link"></textarea>
                    </div>
                    <div class="mb-3">
                    ...
                    <button type="submit" class="btn btn-primary" name="submit">Tambah Record Pertemuan</button>
                </form>
            </div>
        </div>
    </div>
                ...
                while ($fetch = mysqli_fetch_assoc($result)) {
                    ...
                    echo '<div class="card my-3">
                                <div class="card-body">';
                                    ...
                                    echo '
                                    <h5 class="card-title">' . $fetch["pertemuan"] . '</h5>
                                    <p class="card-text">' . $fetch["link"] . '</p>
                                    <button type="button" class="btn btn-primary edit" data-bs-toggle="modal" data-bs-target="#exampleModal" id="' . $fetch["rno"] . '" >Edit</button>
                                    <a href="./deleterecord.php?id=' . $fetch["rno"] . '" class="btn btn-danger">Delete</a>
                                </div>
                                </div>
                            </div>';
                }
                ...
                if ($noRecord) {
                    echo '<div class="card my-3">
                                <div class="card-body">
                                    ...
                                    <p class="card-text">Belum ada record pertemuan nih, yuk add dulu diatas</p>
                                </div>
                                </div>
                            </div>';
                }
    ...
    <script>
        ...
        const editPertemuan = document.getElementById('editpertemuan');
        const editLink = document.getElementById('editlink');
        const hiddenInput = document.getElementById('hidden');
        edit.forEach(element => {
            element.addEventListener('click', () => {
                const pertemuanText = element.parentElement.children[1].innerText;
                const linkText = element.parentElement.children[2].innerText;
                editPertemuan.value = pertemuanText;
                editLink.value = linkText;
